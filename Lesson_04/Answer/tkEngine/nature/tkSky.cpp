...
namespace tkEngine{
	...
	void CSky::Init()
	{
		...
		skinModelData.LoadModelData("Assets/presetModelData/Sky.X", NULL);
		sunModelData.LoadModelData("Assets/presetModelData/sun.x", NULL);
		...
		if (mat) {
			mat->SetTexture(CSkinModelMaterial::enTextureShaderHandle_SkyCubeMap, *TextureResources().Load("Assets/presetModelData/skyCubeMap.dds", true));
