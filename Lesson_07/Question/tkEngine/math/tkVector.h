...
namespace tkEngine{
	...
	public:
		...
		*/
		...
		friend CVector3 operator-(const CVector3& rhs, const CVector3& lhs);
		friend CVector3 operator-(const CVector3& rhs, float s);
	};
	/*!
	 *@brief	4要素のベクトルクラス。
	 */
	...
	public:
		...
		*/
		...
		{
			SetRotation(axis, CMath::DegToRad(angle));
		}
		/*!
	...
	static inline CVector3 operator-(const CVector3& rhs, const CVector3& lhs)
	{
		CVector3 ret;
		ret.x = rhs.x - lhs.x;
		ret.y = rhs.y - lhs.y;
	...
	static CVector3 operator*(const CVector3& rhs, float s)
	{
		CVector3 ret;
		ret.x = rhs.x * s;
		ret.y = rhs.y * s;
